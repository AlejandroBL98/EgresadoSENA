...
namespace Siena.Models
{
    public class RegistroUsuario
    {
        ...
        public int Insertar(Usuario usr)
        {
            ...
            comando.Parameters["@fechaingreso"].Value = DateTime.Today;
        ...
        public List<Usuario> RecupearTodos()
        {
            ...
            SqlCommand com = new SqlCommand("SELECT usu_documento,usu_tipodoc,usu_nombre,usu_celular,usu_email,usu_genero,usu_aprendiz,usu_egresado,usu_areaformacion,usu_fechaegresado,usu_direccion,usu_barrio,usu_ciudad,usu_departamento,usu_fechaingreso FROM es_usuarios ORDER BY usu_nombre ASC", con);
            con.Open();
            ...
            while (registros.Read())
            {
                Usuario usr = new Usuario
                {
                    ...
                    FechaIngreso = DateTime.Parse(registros["usu_fechaingreso"].ToString()),
        ...
        {
            Conectar();
            SqlCommand comando = new SqlCommand("SELECT usu_documento,usu_tipodoc,usu_nombre,usu_celular,usu_email,usu_genero,usu_aprendiz,usu_egresado,usu_areaformacion,usu_fechaegresado,usu_direccionusu_barrio,usu_ciudad,usu_departamento,usu_fechaingreso FROM Usuarios WHERE documento=@documento", con);
            ...
            if (registros.Read())
            {
                ...
                usuario.FechaIngreso = DateTime.Parse(registros["fechaRegistro"].ToString());
            }
            con.Close();
            return usuario;
        }
        ...
        {
            Conectar();
            SqlCommand comando = new SqlCommand("UPDATE es_usuarios set tipoDocumento=@tipodocumento,nombre=@nombre,celular=@celular,email=@email,genero=@genero,aprendiz=@aprendiz,egresado=@egresado,areaFormacion=@areaformacion,fechaEgresado=@fechaegresado,direccion=@direccion,barrio=@barrio,ciudad=@ciudad,departamento=@departamento,fechaingreso=@fechaingreso WHERE documento=@documento", con);
            comando.Parameters.Add("@documento", SqlDbType.Int);
            comando.Parameters.Add("@tipodocumento", SqlDbType.VarChar);
            comando.Parameters.Add("@nombre", SqlDbType.VarChar);
            comando.Parameters.Add("@celular", SqlDbType.Int);
            ...
            comando.Parameters["@ciudad"].Value = usr.Ciudad;
            comando.Parameters["@departamento"].Value = usr.Departamento;
            comando.Parameters["@fecharegistro"].Value = usr.FechaIngreso;
